    ...
    def __init__(self,
        ...
        for i in range(len(layers) - 1):
            if layers[i].output_size != layers[i + 1].input_size:
                raise InputValidationError(f"Layer {i} and {i+1} can't connect.")
        ...
        for i in range(self._layer_count):
            if i == self._layer_count - 1: # final layer
                ...
                continue
            self._layers[i].build(is_final=False)
        ...
        for _ in range(epoch):
            ...
            for i in reversed(range(self._layer_count)):
                if i == self._layer_count - 1:
                    self._layers[i].backward(act_grad=act_grad, is_final=True)
                if i == 0:
                    self._layers[i].backward(act_grad=act_grad, is_first=True)
                else:
                    self._layers[i].backward(act_grad=act_grad)
